...
function render(){
 ...
 $("simTable").innerHTML=rows.map(r=>`<tr><td>${r.q}</td><td>${money(r.cost)}</td><td>${money(r.price)}</td><td>${money(r.profit)}</td></tr>`).join("");
 drawChart(rows);
 renderPreview(c);renderHistory();
}
function drawChart(rows){
 const canvas=$("chart"),ctx=canvas.getContext("2d"),dpr=devicePixelRatio||1,w=canvas.clientWidth,h=260;canvas.width=w*dpr;canvas.height=h*dpr;ctx.scale(dpr,dpr);ctx.clearRect(0,0,w,h);
 if(!rows.length)return;const pad=30,max=Math.max(...rows.map(r=>r.price)),min=Math.min(...rows.map(r=>r.price));const range=max-min||1;
 ctx.beginPath();rows.forEach((r,i)=>{const x=pad+i*(w-pad*2)/(rows.length-1),y=h-pad-(r.price-min)/range*(h-pad*2);i?ctx.lineTo(x,y):ctx.moveTo(x,y)});ctx.strokeStyle=getComputedStyle(document.body).getPropertyValue("--accent")||"#2563eb";ctx.lineWidth=3;ctx.stroke();
}
function renderPreview(c){$("quotePreview").innerHTML=`<h3>Prévia do orçamento</h3><p><b>Cliente:</b> ${esc($("customer").value)}</p><p><b>Produto:</b> ${esc($("productName").value)}</p><p><b>Quantidade:</b> ${c.q}</p><p><b>Preço unitário:</b> ${money(c.sale)}</p><p><b>Total:</b> <b>${money(c.total)}</b></p><p><b>Lucro estimado:</b> ${money(c.profit)}</p><p>${esc($("notes").value)}</p>`}
...
$("shareQuote").onclick=async()=>{const c=calc(),text=`ORÇAMENTO DE IMPRESSÃO 3D\nCliente: ${$("customer").value||"-"}\nProduto: ${$("productName").value||"-"}\nQuantidade: ${c.q}\nPreço unitário: ${money(c.sale)}\nTotal: ${money(c.total)}\nLucro estimado: ${money(c.profit)}`;if(navigator.share){await navigator.share({title:"Orçamento 3D Print Pro",text})}else{await navigator.clipboard.writeText(text);alert("Resumo copiado para a área de transferência.")}};
